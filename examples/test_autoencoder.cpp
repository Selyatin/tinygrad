...
int main(int argc, char **argv) {
    ...
    TGMatrix *input = create_guarded_TGMatrix_with_random_elements(1, n_features, 0.0, 1.0);

    // Create a TGMatrix for holding the target values (a matrix with n_features element, R^d)
    TGMatrix *target = create_guarded_TGMatrix_with_random_elements(1, n_features, 0.0, 1.0);
    ...
    while(epochs > 0) {
        ...
        for(int example_index=0; example_index<ds.records; example_index++){
            ...
            TGMatrix *result = ae.evaluate(input);
